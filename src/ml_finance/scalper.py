# scalper.py (packaged)
from ml_finance.broker_api import *
import numpy as np
    ...
    while True:  # trade loop
        for instrument_name in instruments:
            ...
            if inventory == 0 and bu == 0 and au == 0:
                ...
                api.buy(instrument_name, trade_units, bid_price)
                api.sell(instrument_name, trade_units, ask_price)
            if margin_used < 10:  # critical mm equation
                if net == 0 and even_entry_required:
                    api.buy(instrument_name, trade_units, bid_price)
                    api.sell(
                        instrument_name, trade_units, ask_price
                    )  # disable to slow down
                if net > 0:
                    api.buy(instrument_name, net, bid_price)
                if net < 0:
                    api.sell(instrument_name, net, ask_price)
