...
try:  # pragma: no cover
    ...
    oanda_trades = types.SimpleNamespace(OpenTrades=_EP, TradeDetails=_EP)
    oanda_instruments = types.SimpleNamespace(InstrumentsCandles=_EP)
    oanda_transactions = types.SimpleNamespace()
    oanda_pricing = types.SimpleNamespace(PricingInfo=_EP)
...
class oanda_api(dict):
            ...
            while True:
                try:
                    ...
                    if self._is_maintenance_error(e):
                        print(
                            f"OANDA maintenance detected; retrying in 30 seconds. Error: {e}"
                        )
                        sleep(30)
                        continue
                    raise
    ...
    def _log_trade_as_fill(self, t):
        try:
            ...
            self._append_fill_row(
                path, [ts, instr, trade_id, side, int(units), price, rolling_pl, nav]
            )
    ...
    def start_fill_logger(self, path="logs/fills.csv", poll_interval=5.0):
        ...
        try:
            ...
            if self["_fill_logger_thread"] and self["_fill_logger_thread"].is_alive():
                return True
            th = threading.Thread(
                target=self._fill_logger_loop, args=(float(poll_interval),), daemon=True
            )
    ...
    def _poll_transactions_for_realized(self):
        ...
        try:
            ...
            req = oanda_transactions.TransactionsSinceID(
            ...
            for tx in txs:
                try:
                    ...
                    if tid and (
                        ...
                        or (
                            tid.isdigit()
                            and (not str(max_id).isdigit() or int(tid) > int(max_id))
                        )
                    ):
                        max_id = tid
                except Exception:
                ...
                if realized is None:
                    continue
                try:
                    self["_realized_pl_cum"][instr] = float(
                        self["_realized_pl_cum"].get(instr, 0.0)
                    ) + float(realized)
                except Exception:
                    pass
    ...
    def poll_and_log_pnl(self):
        ...
        for instr in instruments:
            d = agg.get(
                instr,
                {"net_units": 0.0, "abs_units": 0.0, "cb_num": 0.0, "unreal": 0.0},
            )
            try:
            ...
            realized_cum = (
                float(self["_realized_pl_cum"].get(instr, 0.0))
                if self.get("_realized_pl_cum")
                else 0.0
            )
            row = [
                ts,
    ...
    def start_pnl_logger(self, path="logs/pnl.csv", poll_interval=5.0):
        ...
        try:
            ...
            if self["_pnl_logger_thread"] and self["_pnl_logger_thread"].is_alive():
                return True
            th = threading.Thread(
                target=self._pnl_logger_loop, args=(float(poll_interval),), daemon=True
            )
        ...
        try:
            ...
            for t in trades:
                try:
                    ...
                    notional_usd = self.convert_currency(
                        quote_ccy, "USD", notional_quote
                    )
    ...
    def _get_precisions(self, instrument):
        ...
        try:
            ...
            r = oanda_accounts.AccountInstruments(
                accountID=self["account_id"], params=params
            )
    ...
    def list_pending_orders(self, instrument=None, include_attached=True):
        ...
        try:
            ...
            pend = tmp
            instr = (
                str(instrument).upper().replace("-", "_").strip()
                if instrument
                else None
            )
            for od in pend:
                od_type = str(od.get("type", "")).upper()
                if not include_attached and od_type in (
                    "STOP_LOSS",
                    "TAKE_PROFIT",
                    "TRAILING_STOP_LOSS",
                ):
                        ...
                        out.append(
                            {
                                ...
                                "instrument": ins,
                                "type": od_type,
                                "tradeID": od.get("tradeID") or od.get("tradeId"),
                            }
                        )
                    else:
                        # For attached orders with no instrument, try resolving via TradeDetails
                        if include_attached and od_type in (
                            "STOP_LOSS",
                            "TAKE_PROFIT",
                            "TRAILING_STOP_LOSS",
                        ):
                            ...
                            if tid:
                                try:
                                    td = oanda_trades.TradeDetails(
                                        accountID=self["account_id"], tradeID=str(tid)
                                    )
    ...
    def current_price(self, instrument="EUR_USD"):
        try:
            ...
            r = oanda_pricing.PricingInfo(
                accountID=self["account_id"], params={"instruments": query_instr}
            )
            rv = self["api"].request(r)
    ...
    def convert_currency(
        self, from_currency, to_currency, amount=1.0, price_type="mid"
    ):
        """
    ...
    def get_symbol_history(self, instrument="EUR_USD", data_rate="M1", periods=250):
        if isinstance(instrument, list):
            ...
            for instr in instrument:
                rDict[instr] = self.get_candle_history(
                    instrument=instr.upper(),
                    data_rate=data_rate.upper(),
                    periods=periods,
                )
            return rDict
        else:
            return self.get_candle_history(
                instrument=instrument, data_rate=data_rate, periods=periods
            )
    ...
    def get_candle_history(self, instrument="EUR_USD", data_rate="M1", periods=250):
        ...
        try:
            ...
            granularity = granularity_map.get(
                str(data_rate).upper(), str(data_rate).upper()
            )
            ...
            r = oanda_instruments.InstrumentsCandles(
    ...
    def get_all_resting_bids_list(
        self, instrument="EUR_USD", scale=None, round_price=True, sort="asc"
    ):
        """
    ...
    def get_bid_ask_candles(
        ...
        try:
            ...
            req_count = count + (
                0 if include_incomplete else 1
            )  # over-fetch one to ensure enough completed bars
            params = {
            ...
            if not rows:
                ...
                return (
                    []
                    if not (as_dataframe and "pd" in globals())
                    else pd.DataFrame(
                        columns=[
                            "time",
                            "complete",
    ...
    def get_market_order_volume_ewma(
        self,
        instrument="EUR_USD",
        granularity="M1",
        samples=1000,
        span=36,
        order_tick_ratio=1.0,
        as_dataframe=True,
    ):
        """
        ...
        alpha = 2.0 / (s + 1.0)
        try:
            df = self.get_candle_history(
                instrument=instrument, data_rate=str(granularity).upper(), periods=n
            )
        except Exception:
            df = None
        try:
            ...
            if as_dataframe and "pd" in globals():
                try:
                    out = {"market_volume": mv, "ewma": ew, "volume": vol_seq}
            ...
            # final fallback
            return (
                pd.DataFrame(columns=["time", "volume", "market_volume", "ewma"])
                if (as_dataframe and "pd" in globals())
                else []
            )
    ...
    def place_order(
        ...
        if not self._has_instrument(instr):
            logging.error(
                f"Failed to place order: invalid instrument '{instrument}' for account {self['account_id']}"
            )
            return None
        # user function
        data = {
        ...
        if trailing_stop_pips is not None:
            try:
                ...
                if dist > 0:
                    ...
                    try:
                        if min_pips is not None and float(trailing_stop_pips) < float(
                            min_pips
                        ):
                            print(
                                f"Adjusted trailing_stop_pips from {trailing_stop_pips} to broker minimum ≈ {round(min_pips, 5)} pips for {instr} (distance {dist})."
                            )
                    except Exception:
                        pass
            except Exception:
        ...
        try:
            ...
            if wait and rv is not None:
                try:
                    return self.wait_for_order_completion(
                        rv, timeout=timeout, poll_interval=poll_interval
                    )
    ...
    def cancel_orders(self):
        """
        Cancel all pending orders for the account (best-effort). This is a safe
        no-op in offline/sandbox environments and will attempt to cancel any
        pending orders if the API supports it.
        """
        try:
            # Attempt to list pending orders and cancel them if an OrderCancel
            # endpoint is available in the client shim.
            try:
                r = oanda_orders.OrdersPending(accountID=self["account_id"])
                rv = self["api"].request(r)
                orders = rv.get("orders", []) if isinstance(rv, dict) else []
            except Exception:
                orders = []
            for od in orders:
                oid = od.get("id")
                try:
                    rc = oanda_orders.OrderCancel(
                        accountID=self["account_id"], orderID=str(oid)
                    )
                    self["api"].request(rc)
                except Exception:
                    # Ignore individual cancel failures and continue
                    pass
        except Exception:
            # Swallow any errors to keep cancel_orders safe to call
            pass
        return True
