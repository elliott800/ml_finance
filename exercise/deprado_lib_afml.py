...
pcRank=np.array([1,2,4,3]) # PCA ran
...
'''#———————————————————————————————————————
#---------------------------------------
#this is what appears to be an incomplete function
...
#META-LABELING
def getEvents2(close,tEvents,ptSl,trgt,minRet,numThreads,t1=False,side=None):
    #1) get target
    trgt=trgt.loc[tEvents]
    trgt=trgt[trgt>minRet] # minRet
    #2) get t1 (max holding period)
...
def get_csvs(csv_folder=PACKAGEDIR+'csvs\\assets'):
    #EPH
    db_location=csv_folder+'\\'
    db_file_names=os.listdir(db_location)
    csvs={}
    for each in db_file_names:
        original=each
        each=each.replace('.csv','').lower()
        csvs[each]=pd.read_csv(db_location+original)
        columns_dict={}
        for every in csvs[each].columns:
            columns_dict[every]=every.lower()
        csvs[each]=csvs[each].rename(columns=columns_dict)
        csvs[each]=csvs[each].rename(columns={'local time':'date'})
    for each in os.listdir(db_location):
        each=each.replace("=X",'')
        os.rename(db_location+original,db_location+original.lower())
    return csvs

def format_csv_df_dates(df):
    rList=[]
    for i in range(len(df.date)):
        date=str(df.date.iloc[i])
        rList.append(date[:4]+'-'+date[4:6]+'-'+date[6:])
    formatted_dates=pd.to_datetime(pd.Series(rList))
    df.index=formatted_dates
    del df['date']
    return df

def __main():
    asset=get_csvs()['spxcsh']
    asset=format_csv_df_dates(asset)
    #print(dropLabels(getBins(getEvents(asset['close'],symetricCusum(asset['close'],250),getDailVol(asset['close'],span0=100)),asset['close'])))
    #print(dropLabels(getBins(getEvents(asset['close'],symetricCusum(asset['close'],250),getDailVol(asset['close'],span0=100)),asset['close'])))
    side=getBins(getEvents(asset['close'],symetricCusum(asset['close'],250),getDailVol(asset['close'],span0=100)),asset['close'])#['bin']#working without #[bin]
    side['side']=side['bin']
    #print(side['side'])
    print(metaGetEvents(asset['close'],symetricCusum(asset['close'],250),[1,1,1],getDailVol(asset['close'],span0=100),side=side))
    #when introducing side into metaGetEvents it throws an error
    #print(metaGetEvents(asset['close'],symetricCusum(asset['close'],250),getDailVol(asset['close'],span0=100),side=side))
    #print(dropLabels(metaGetBins(metaGetEvents(asset['close'],symetricCusum(asset['close'],250),getDailVol(asset['close'],span0=100),side=side),asset['close'])))
    input()
